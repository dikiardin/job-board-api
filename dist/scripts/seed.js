...
async function seed() {
    ...
    await clearDatabase();
    // Users
    const adminPassword = await (0, hashPassword_1.hashPassword)("admin123");
    const userPassword = await (0, hashPassword_1.hashPassword)("user12345");
    ...
    async function addPreselection(jobId, questionCount = 5, passingScore = 4) {
    ...
    await prisma.interview.create({
        data: {
            ...
            status: prisma_1.InterviewStatus.CANCELLED,
        },
    });
    await prisma.interview.create({
        data: {
            ...
            status: prisma_1.InterviewStatus.NO_SHOW,
        },
    });
    // Subscriptions
    ...
    await prisma.payment.create({
        data: {
            ...
            status: prisma_1.PaymentStatus.REJECTED,
            amount: "99000.00",
    ...
    await prisma.payment.create({
        data: {
            ...
            paymentMethod: prisma_1.PaymentMethod.GATEWAY,
            status: prisma_1.PaymentStatus.EXPIRED,
            amount: "199000.00",
    ...
    await prisma.jobShare.create({
        data: { userId: alice.id, jobId: jobFrontend.id, platform: prisma_1.SharePlatform.LINKEDIN, sharedUrl: "https://linkedin.com/demo" },
    });
    await prisma.jobShare.create({
        data: { userId: bob.id, jobId: jobBackend.id, platform: prisma_1.SharePlatform.WHATSAPP, sharedUrl: "https://wa.me/demo" },
    });
    await prisma.jobShare.create({
        data: { userId: alice.id, jobId: jobBackend.id, platform: prisma_1.SharePlatform.FACEBOOK, sharedUrl: "https://facebook.com/demo" },
    });
    await prisma.jobShare.create({
        data: { userId: charlie.id, jobId: jobFrontend.id, platform: prisma_1.SharePlatform.TWITTER, sharedUrl: "https://x.com/demo" },
    });
    // Skill assessments
