...
class ChangeEmailService {
    static async changeEmail(userId, newEmail) {
        ...
        const company = await company_repository_1.CompanyRepo.findByAdminId(userId);
        if (company) {
            await company_repository_1.CompanyRepo.updateCompany(company.id, { email: newEmail });
        }
