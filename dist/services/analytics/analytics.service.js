...
class AnalyticsService {
    ...
    static async conversionFunnel(params) {
        ...
        await this.assertCompanyOwnership(companyId, requesterId, requesterRole);
        const { from, to } = parseRange(query);
        const funnelData = await analytics_repository_1.AnalyticsRepository.conversionFunnelData({ companyId, ...(from && { from }), ...(to && { to }) });
        return funnelData;
    }
    static async retention(params) {
        ...
        await this.assertCompanyOwnership(companyId, requesterId, requesterRole);
        const { from, to } = parseRange(query);
        const retentionData = await analytics_repository_1.AnalyticsRepository.retentionData({ companyId, ...(from && { from }), ...(to && { to }) });
    ...
    static async performance(params) {
        ...
        await this.assertCompanyOwnership(companyId, requesterId, requesterRole);
        const { from, to } = parseRange(query);
        const performanceData = await analytics_repository_1.AnalyticsRepository.performanceData({ companyId, ...(from && { from }), ...(to && { to }) });
