...
describe('ApplicationService.submitApplication', () => {
    ...
    it('rejects if job is not open', async () => {
        ...
        await expect(application_service_1.ApplicationService.submitApplication(2, 10, dummyFile)).rejects.toMatchObject({ message: expect.stringMatching(/not open/i) });
    });
    it('rejects duplicate application', async () => {
        ...
        await expect(application_service_1.ApplicationService.submitApplication(2, 10, dummyFile)).rejects.toMatchObject({ message: expect.stringMatching(/already applied/i) });
    });
    it('enforces preselection when active and result missing', async () => {
        ...
        await expect(application_service_1.ApplicationService.submitApplication(2, 10, dummyFile)).rejects.toMatchObject({ message: expect.stringMatching(/complete the pre-selection test/i) });
    });
    it('rejects when score below passing', async () => {
        ...
        await expect(application_service_1.ApplicationService.submitApplication(2, 10, dummyFile)).rejects.toMatchObject({ message: expect.stringMatching(/does not meet the passing/i) });
    });
    it('creates application when no test or passed', async () => {
        ...
        await application_service_1.ApplicationService.submitApplication(2, 10, dummyFile, 20000000);
