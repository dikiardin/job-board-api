...
class App {
    ...
    configure() {
        ...
        // CORS configuration
        const devOrigins = new Set([process.env.FE_URL, "http://localhost:3000", "http://127.0.0.1:3000", "https://job-board-app-xi.vercel.app"].filter(Boolean));
        this.app.use((0, cors_1.default)({
            origin: (origin, callback) => {
                ...
                if (process.env.NODE_ENV === "production") {
                    const allowed = ["https://job-board-app-xi.vercel.app", process.env.FE_URL].filter(Boolean);
