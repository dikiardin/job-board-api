...
export class CVDownloadController {
  ...
  async downloadCV(req: AuthRequest, res: Response) {
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="CV_${cv.id}.pdf"`
      );
  ...
  async publicDownloadCV(req: Request, res: Response) {
    try {
      const cvIdParam = req.params.id;
      if (!cvIdParam) {
        return res.status(400).json({ message: "CV ID is required" });
  ...
  async getTemplates(req: AuthRequest, res: Response) {
    try {
      const allTemplates = cvService.getAvailableTemplates();
      ...
      const availableTemplates = allTemplates.filter((template: any) =>
        userLimits?.templatesAccess.includes(template.id)
      );
      ...
      res.status(200).json({
        ...
        subscription: {
          ...
          cvLimit:
            userLimits?.cvGenerationLimit === -1
              ? "Unlimited"
...
export const cvDownloadController = new CVDownloadController();
