...
export class AssessmentManagementController {
  ...
  public static async getAssessments(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      res.status(200).json({ success: true, message: "Assessments retrieved successfully", data: result });
    } catch (error) {
      next(error);
    }
  }
  public static async getAssessmentById(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      res.status(200).json({ success: true, message: "Assessment retrieved successfully", data: assessment });
    } catch (error) {
      next(error);
    }
  }
  public static async getAssessmentBySlug(req: Request, res: Response, next: NextFunction) {
    try {
      const { role } = res.locals.decrypt;
      const slug = (req.params as any).slug as string;
      const assessment = await AssessmentCreationService.getAssessmentBySlug(slug, role);
      res.status(200).json({ success: true, message: "Assessment retrieved successfully", data: assessment });
    } catch (error) {
      next(error);
    }
  ...
  public static async updateAssessment(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await AssessmentCreationService.updateAssessment(assessmentId, userId, { title, description, category, badgeTemplateId, passScore, questions });
  ...
  public static async getDeveloperAssessments(req: Request, res: Response, next: NextFunction) {
    try {
      const { userId, role } = res.locals.decrypt;
      const page = parseInt(req.query.page as string) || 1;
      const limit = parseInt(req.query.limit as string) || 10;
      const assessments = await AssessmentCreationService.getAssessments(page, limit);
      res.status(200).json({ success: true, message: "Developer assessments retrieved successfully", data: assessments });
    } catch (error) {
      next(error);
    }
  }
