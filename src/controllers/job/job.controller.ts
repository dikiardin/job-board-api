...
export class JobController {
  static async create(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const job = await JobService.createJob({ companyId, requesterId: requester.userId, requesterRole: requester.role, body: req.body });
  ...
  static async list(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const data = await JobService.listJobs({ companyId, requesterId: requester.userId, requesterRole: requester.role, query });
  ...
  static async update(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const jobId = req.params.jobId as string;
      const requester = res.locals.decrypt as { userId: number; role: UserRole };
      const job = await JobService.updateJob({ companyId, jobId, requesterId: requester.userId, requesterRole: requester.role, body: req.body });
  ...
  static async applicantsList(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (typeof ageMin === "string" && ageMin.trim() !== "") {
        ...
        if (age < 0) return res.status(400).json({ success: false, message: "Age minimum cannot be negative" });
        query.ageMin = age;
      }
      if (typeof ageMax === "string" && ageMax.trim() !== "") {
        const age = Number(ageMax);
        if (age < 0) return res.status(400).json({ success: false, message: "Age maximum cannot be negative" });
        query.ageMax = age;
      }
      ...
      if (typeof limit === "string" && limit.trim() !== "") {
        ...
        if (limitNum < 0) return res.status(400).json({ success: false, message: "Limit cannot be negative" });
        query.limit = Math.min(limitNum, 100);
      }
      if (typeof offset === "string" && offset.trim() !== "") {
        const offsetNum = Number(offset);
        if (offsetNum < 0) return res.status(400).json({ success: false, message: "Offset cannot be negative" });
  ...
  static async updateApplicantStatus(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const requester = res.locals.decrypt as { userId: number; role: UserRole };
      const data = await JobApplicantsService.updateApplicantStatus({ companyId, jobId, applicationId, requesterId: requester.userId, requesterRole: requester.role, body: req.body });
      res.status(200).json({ success: true, data });
    } catch (error) { next(error); }
  }
}
