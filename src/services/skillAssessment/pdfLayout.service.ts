...
export class PDFLayoutService {
  public static async generateCertificatePDF(data: {
    userName: string; userEmail: string; assessmentTitle: string; assessmentDescription?: string;
    score: number; totalQuestions: number; completedAt: Date; userId: number; certificateCode: string; badgeIcon?: string; badgeName?: string;
  }): Promise<Buffer> {
    const doc = new PDFKit.default({ size: "A4", layout: "landscape", margin: 40 });
  ...
  private static addLogo(doc: any, pageWidth: number) {
    try {
      ...
      if (fs.existsSync(logoPath)) {
        doc.image(logoPath, pageWidth - 140, 50, { fit: [70, 40], align: "center", valign: "center" });
      }
    } catch (error) {
  ...
  private static addHeader(doc: any, centerX: number) {
    ...
    doc.fontSize(36).fillColor(primaryColor).font("Helvetica-Bold").text("CERTIFICATE", centerX - 120, 80, { width: 240, align: "center" });
    doc.fontSize(24).fillColor(secondaryColor).text("OF ACHIEVEMENT", centerX - 120, 120, { width: 240, align: "center" });
    const goldColor = "#FFD700";
    doc.moveTo(centerX - 150, 160).lineTo(centerX + 150, 160).lineWidth(2).stroke(goldColor);
  }
  ...
  private static addUserInfo(doc: any, centerX: number, data: any) {
    ...
    if (data.badgeName) PDFLayoutHelper.addBadgeInfo(doc, centerX, data.badgeName);
  }
  ...
  private static addScoreInfo(doc: any, centerX: number, data: any) {
    ...
    doc.fontSize(18).fillColor(primaryColor).font("Helvetica-Bold").text(`Score: ${scorePercentage}/100 (${scorePercentage}%)`, centerX - 200, scoreY, { width: 400, align: "center" });
  }
  ...
  private static addDateAndSignature(doc: any, centerX: number, completedAt: Date, data: any) {
    ...
    doc.fontSize(14).fillColor(lightGray).font("Helvetica").text(`Completed on: ${formattedDate}`, centerX - 200, dateY, { width: 400, align: "center" });
  }
  ...
  private static async addQRCode(doc: any, centerX: number, certificateCode: string, data: any) {
    ...
    doc.fontSize(14).fillColor(lightGray).font("Helvetica").text(`Certificate Code: ${certificateCode}`, centerX - 200, codeY, { width: 400, align: "center" });
    const verificationUrl = `${process.env.FE_URL || "http://localhost:3000"}/verify-certificate/${certificateCode}`;
    try {
      const qrCodeBuffer = await QRCode.toBuffer(verificationUrl, { type: "png", width: 50, margin: 1, color: { dark: "#2D3748", light: "#FFFFFF" } });
      ...
      doc.fontSize(6).fillColor(primaryColor).font("Helvetica-Bold").text("Scan to Verify", qrX, qrY + 53, { width: 50, align: "center" });
      doc.fontSize(5).fillColor(lightGray).font("Helvetica").text(verificationUrl, centerX - 60, qrY + 62, { width: 120, align: "center" });
      doc.link(centerX - 60, qrY + 62, 120, 8, verificationUrl);
      doc.fontSize(9).fillColor(primaryColor).font("Helvetica-Bold").text("WORKOO JOB BOARD", 50, doc.page.height - 60);
    } catch (error) {
      const qrX = centerX - 25;
      const qrY = data.badgeName ? 470 : 465;
      doc.rect(qrX, qrY, 50, 50).lineWidth(1).stroke(lightGray);
      doc.fontSize(6).fillColor(lightGray).text("Verify at:", qrX, qrY + 53, { width: 50, align: "center" });
      doc.fontSize(5).text(verificationUrl, centerX - 60, qrY + 62, { width: 120, align: "center" });
      doc.link(centerX - 60, qrY + 62, 120, 8, verificationUrl);
      doc.fontSize(9).fillColor(primaryColor).font("Helvetica-Bold").text("WORKOO JOB BOARD", 50, doc.page.height - 60);
    }
  }
