...
export class AssessmentValidationService {
  ...
  private static validateSingleQuestion(question: any, index: number) {
    ...
    if (question.answer && !question.options.includes(question.answer)) {
      throw new CustomError(
        `Question ${index} answer must be one of the options`,
        400
      );
    }
  }
  ...
  public static validateTimeLimit(startedAt: Date, finishedAt: Date) {
    ...
    if (minutesDiff > 31) {
      throw new CustomError(
        `Assessment submission time exceeded maximum allowed duration of 30 minutes. Time taken: ${
          Math.round(minutesDiff * 100) / 100
        } minutes`,
        400
      );
    }
  ...
  public static validateAnswerCount(
    answersCount: number,
