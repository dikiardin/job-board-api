export class AssessmentScoringService {
  ...
  public static getPerformanceLevel(score: number): {
    level: string;
    color: string;
    description: string;
  } {
  ...
  public static getImprovementRecommendations(score: number): string[] {
    if (score >= 95) {
      return [
  ...
  public static getDetailedScoreBreakdown(
    questions: Array<{ answer: string; category?: string }>,
    userAnswers: Array<{ questionId: number; answer: string }>
  ) {
    const breakdown = {
      overall: this.calculateScore(questions, userAnswers),
      categories: new Map<string, { correct: number; total: number; percentage: number }>(),
    };
