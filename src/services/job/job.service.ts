...
export class JobService {
  ...
  static validateJobPayload(payload: any, isUpdate = false) {
    ...
    if (payload?.deadline) {
      ...
      else if (!isUpdate && d.getTime() < Date.now()) errors.push("deadline cannot be in the past");
    }
  ...
  static async createJob(params: {
    companyId: string; requesterId: number; requesterRole: UserRole;
    body: { title: string; description: string; banner?: string | null; category: string; city: string;
      salaryMin?: number | null; salaryMax?: number | null; tags?: string[]; deadline?: string | Date | null; isPublished?: boolean; };
  }) {
  ...
  static async updateJob(params: { companyId: string; jobId: string; requesterId: number; requesterRole: UserRole; body: any; }) {
  ...
  static async listPublishedJobs(params: {
    ...
    const { query } = params;
    const repoQuery: { title?: string; category?: string; city?: string; sortBy?: "createdAt" | "deadline"; sortOrder?: "asc" | "desc"; limit?: number; offset?: number; } = {};
    if (typeof query.title === "string") repoQuery.title = query.title;
    if (typeof query.category === "string") repoQuery.category = query.category;
  ...
  static async jobDetail(params: { companyId: string; jobId: string; requesterId: number; requesterRole: UserRole }) {
    ...
    const applicants = ((job as any).applications ?? []).map((a: any) => {
      ...
      if (test) {
        ...
        if (result) preselectionPassed = passingScore != null ? result.score >= passingScore : true;
        else preselectionPassed = false;
      }
      return {
    ...
    return {
      id: job.id, title: job.title, description: job.description, banner: (job as any).bannerUrl ?? null,
      category: job.category, city: job.city, employmentType: job.employmentType ?? null, experienceLevel: job.experienceLevel ?? null,
      salaryMin: job.salaryMin, salaryMax: job.salaryMax, tags: job.tags, deadline: (job as any).applyDeadline ?? null,
      isPublished: job.isPublished, createdAt: job.createdAt,
      applicantsCount: (job as any)._count?.applications ?? ((job as any).applications?.length ?? 0), applicants,
    };
  }
}
