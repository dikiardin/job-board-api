export class PDFHeaderService {
  ...
  generateHeader(doc: any, cvData: any, pageWidth: number, margin: number): number {
    ...
    if (cvData.additionalInfo?.objective) {
      const firstLine =
        cvData.additionalInfo.objective.split(".")[0] || "Professional";
  ...
  generateProjects(doc: any, cvData: any, yPosition: number, margin: number, contentWidth: number): number {
    if (
      cvData.additionalInfo?.projects &&
      ...
      cvData.additionalInfo.projects.forEach((project: any, index: number) => {
...
export const pdfHeaderService = new PDFHeaderService();
