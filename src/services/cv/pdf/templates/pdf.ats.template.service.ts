...
import { pdfSectionsHelperService } from './pdf.sections.helper.service';

export class PDFATSTemplateService {
  generateATSTemplate(doc: any, cvData: CVData) {
    ...
    let yPosition = pdfCoreService.generateHeader(doc, cvData, pageWidth);
    ...
    yPosition = pdfSectionsHelperService.generateWorkExperienceSection(doc, cvData, yPosition, margin, contentWidth);
    yPosition = pdfSectionsHelperService.generateEducationSection(doc, cvData, yPosition, margin, contentWidth);
    yPosition = pdfSectionsHelperService.generateCertificationsSection(doc, cvData, yPosition, margin, contentWidth);
    yPosition = this.generateSkillsSection(doc, cvData, yPosition, margin, contentWidth);
  }

  private generateSummarySection(doc: any, cvData: CVData, yPosition: number, margin: number, contentWidth: number): number {
    if (cvData.additionalInfo?.objective) {
      pdfCoreService.addSectionHeader(doc, 'SUMMARY', yPosition, margin, contentWidth);
      yPosition += 20;
  ...
  private generateProjectsSection(doc: any, cvData: CVData, yPosition: number, margin: number, contentWidth: number): number {
    if (cvData.additionalInfo?.projects && cvData.additionalInfo.projects.length > 0) {
      pdfCoreService.addSectionHeader(doc, 'PROJECT EXPERIENCE', yPosition, margin, contentWidth);
      yPosition += 20;
      
      cvData.additionalInfo.projects.forEach((project, index) => {
        // Check if we need a new page
        yPosition = pdfCoreService.checkNewPage(doc, yPosition, 650);
  ...
  private generateSkillsSection(doc: any, cvData: CVData, yPosition: number, margin: number, contentWidth: number): number {
    ...
    if (allSkills.length > 0) {
      pdfCoreService.addSectionHeader(doc, 'SKILLS', yPosition, margin, contentWidth);
      yPosition += 20;
  ...
  private renderManualCategories(doc: any, customCategories: Record<string, string[]>, margin: number, contentWidth: number, yPosition: number): number {
  ...
  private renderSimpleSkills(doc: any, skills: string[], margin: number, contentWidth: number, yPosition: number): number {
...
export const pdfATSTemplateService = new PDFATSTemplateService();
