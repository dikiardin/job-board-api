...
export class PDFSectionsHelperService {
  generateWorkExperienceSection(doc: any, cvData: CVData, yPosition: number, margin: number, contentWidth: number): number {
    ...
    if (cvData.additionalInfo?.workExperience && cvData.additionalInfo.workExperience.length > 0) {
      ...
      pdfCoreService.addSectionHeader(doc, 'WORK EXPERIENCE', yPosition, margin, contentWidth);
      yPosition += 20;
      ...
      cvData.additionalInfo.workExperience.forEach((workExp: any, index: number) => {
        ...
        yPosition = pdfCoreService.checkNewPage(doc, yPosition, 700);
  ...
  generateEducationSection(doc: any, cvData: CVData, yPosition: number, margin: number, contentWidth: number): number {
    ...
    if (cvData.education || cvData.additionalInfo?.educationDetails) {
      ...
      pdfCoreService.addSectionHeader(doc, 'EDUCATION', yPosition, margin, contentWidth);
      yPosition += 20;
  ...
  generateCertificationsSection(doc: any, cvData: CVData, yPosition: number, margin: number, contentWidth: number): number {
    if (cvData.additionalInfo?.certifications && cvData.additionalInfo.certifications.length > 0) {
      pdfCoreService.addSectionHeader(doc, 'LICENSES & CERTIFICATION', yPosition, margin, contentWidth);
      yPosition += 20;
...
export const pdfSectionsHelperService = new PDFSectionsHelperService();
