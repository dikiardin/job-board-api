export class PDFSectionsService {
  ...
  addEducationSection(doc: any, cvData: any, yPosition: number, margin: number, contentWidth: number): number {
    ...
    if (cvData.education || cvData.additionalInfo?.educationDetails) {
      ...
      if (cvData.additionalInfo?.educationDetails) {
        cvData.additionalInfo.educationDetails.forEach((edu: any, index: number) => {
  ...
  addCertificationsSection(doc: any, cvData: any, yPosition: number, margin: number, contentWidth: number): number {
    if (cvData.additionalInfo?.certifications && cvData.additionalInfo.certifications.length > 0) {
      ...
      cvData.additionalInfo.certifications.forEach((cert: any, index: number) => {
  ...
  addSkillsSection(doc: any, cvData: any, yPosition: number, margin: number, contentWidth: number): number {
    ...
    if (cvData.skills && cvData.skills.length > 0 || cvData.additionalInfo?.skills || cvData.additionalInfo?.skillCategories) {
      ...
      if (cvData.additionalInfo?.skillCategories) {
        ...
        Object.keys(categories).forEach((category: string, index: number) => {
          ...
          if (skills && skills.length > 0) {
            ...
            const skillsText = skills.map((skill: string) => 
              skill.split(' ').map((word: string) => 
...
export const pdfSectionsService = new PDFSectionsService();
