...
export class PreselectionService {
  static async createOrUpdateTest(params: {
    jobId: string | number; requesterId: number; requesterRole: UserRole;
    questions: Array<{ question: string; options: string[]; answer: string }>; passingScore?: number; isActive?: boolean;
  }) {
  ...
  static async submitAnswers(params: {
    applicantId: number; pathApplicantId: number; testId: number; requesterRole: UserRole;
  ...
  static async statusForUser(params: { jobId: string | number; userId: number }) {
    ...
    const test = await PreselectionRepository.getTestByJobId(jobId);
    if (!test || !test.isActive) { return { required: false }; }
