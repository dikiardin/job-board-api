...
export async function getAllUsers(params: { from?: Date; to?: Date }) {
...
export async function getAllApplications(params: { from?: Date; to?: Date }) {
  const { from, to } = params;
...
export async function platformSalaryTrends(params: { from?: Date; to?: Date }) {
  const { from, to } = params;
  const where: Prisma.ApplicationWhereInput = {
...
export async function platformReviewSalaryStats() {
  const reviews = await prisma.companyReview.findMany({ where: { salaryEstimateMin: { not: null }, salaryEstimateMax: { not: null } } });
...
export async function platformJobCategories(params: { from?: Date; to?: Date }) {
  const { from, to } = params;
  const where: Prisma.ApplicationWhereInput = {
    ...(from || to ? { createdAt: { ...(from ? { gte: from } : {}), ...(to ? { lte: to } : {}) } } : {}),
  };
...
export async function platformApplicationStatusCounts(params: { from?: Date; to?: Date }) {
  const { from, to } = params;
  const where: Prisma.ApplicationWhereInput = {
    ...(from || to ? { createdAt: { ...(from ? { gte: from } : {}), ...(to ? { lte: to } : {}) } } : {}),
  };
...
export async function platformTopCities(params: { from?: Date; to?: Date }) {
  const { from, to } = params;
  const where: Prisma.ApplicationWhereInput = {
    ...(from || to ? { createdAt: { ...(from ? { gte: from } : {}), ...(to ? { lte: to } : {}) } } : {}),
  };
...
export async function platformDailyActiveUsers(params: { from?: Date; to?: Date }) {
  const { from, to } = params;
  const where: Prisma.ApplicationWhereInput = {
    ...(from || to ? { createdAt: { ...(from ? { gte: from } : {}), ...(to ? { lte: to } : {}) } } : {}),
  };
...
export async function platformMonthlyActiveUsers(params: { from?: Date; to?: Date }) {
  const { from, to } = params;
  const where: Prisma.ApplicationWhereInput = {
    ...(from || to ? { createdAt: { ...(from ? { gte: from } : {}), ...(to ? { lte: to } : {}) } } : {}),
  };
...
export async function platformSessionMetrics(params: { from?: Date; to?: Date }) {
  return { averageSessionDuration: 300, bounceRate: 0.35, sessionsPerUser: 2.1 };
}

export async function platformPageViews(params: { from?: Date; to?: Date }) {
  return { total: 0, unique: 0, topPages: [] };
}

export async function platformConversionFunnelData(params: { from?: Date; to?: Date }) {
  const { from, to } = params;
  const where: Prisma.ApplicationWhereInput = {
    ...(from || to ? { createdAt: { ...(from ? { gte: from } : {}), ...(to ? { lte: to } : {}) } } : {}),
  };
...
export async function platformRetentionData(params: { from?: Date; to?: Date }) {
  return { day1: 0.85, day7: 0.65, day30: 0.45, cohorts: [] };
}

export async function platformPerformanceData(params: { from?: Date; to?: Date }) {
